...
PLATFORM_FIELDS: dict[str, str] = {
    ...
    # Добавлены после демо 19.06 (по просьбе заказчика):
    "PowerBI": "customfield_13936",          # поле-оценка PowerBI (выверено по файлу)
    "Сервис Самопланирования": "customfield_SERVICE_TBD",  # поля-оценки в Jira пока нет — добавят позже
}

# Синонимы значений селектора Platform (customfield_12613) -> каноническое имя из PLATFORM_FIELDS.
# В выгрузке встречаются варианты написания той же платформы.
PLATFORM_ALIASES: dict[str, str] = {
    "1C ЗУП": "1С ЗУП",      # латинская C (U+0043) вместо кириллической С (U+0421)
    "Команда СНГ": "СНГ",
    "Маркетплейсы": "MP",    # оценка пишется в поле MP (customfield_12608), подтверждено по файлу
}

# Значения селектора, которые НЕ платформы: нет полей-оценок и нет в справочниках Excel/fields.json.
# Их пропускаем при разборе требуемых платформ.
# VRCRM — внешняя «резиновая» команда (ёмкость не считаем), оставляем в игноре.
PLATFORM_IGNORE: set[str] = {
    "VRCRM",
}
